...
module Day03 =
    let private simulateMovements (movement: string) (y, x) =
        ...
        | 'L' -> [ for cx in x .. -1 .. x - dist -> y, cx ], (y, x - dist)
        | 'R' -> [ for cx in x .. x + dist -> y, cx ], (y, x + dist)
        | 'U' -> [ for cy in y .. y + dist -> cy, x ], (y + dist, x)
        | 'D' -> [ for cy in y .. -1 .. y - dist -> cy, x ], (y - dist, x)
    ...
    let solvePart02 (input: string array) = 0
